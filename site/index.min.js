...
user_id:e.user_id,text:e.question,time:e.time,answer:"",answer_time:0,persons:0,status:"routing"});break;case "answer":c.answer=e.answer;c.answer_time=a?e.time:Date.now();e.user_id===this.state.id&&(this.state.answereds[e.question_id]=!0);break;case "dunno":++c.persons;e.user_id===this.state.id&&(this.state.dunnos[e.question_id]=!0);break;case "accept":""!==c.answer&&"routing"===c.status&&e.user_id===c.user_id&&(c.status="accepted",this.state.answereds[c.id]&&++this.state.points);break;case "refuse":""!==
c.answer&&"routing"===c.status&&e.user_id===c.user_id&&(c.status="refused",this.state.answereds[c.id]&&--this.state.points)}}console.log("Client id is: "+this.state.id);console.log("Questions: "+JSON.stringify(this.state.questions));this.state.act=function(a){return function(){h.send(JSON.stringify(a))}};this.forceUpdate()}.bind(this);setInterval(function(){this.forceUpdate()}.bind(this),500)},render:function(){function h(a){return div({className:"centralizer_outer"},div({className:"centralizer_inner"},
